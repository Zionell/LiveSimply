...
const schema = z.object({
	subtitle: z.string({ error: t("inputsErrors.required") }),
	description: z.string({ error: t("inputsErrors.required") }),
});
...
<template>
	<CommonCardWrapper>
		...
		<UForm
			...
			@submit="handleSubmit"
		>
			<UFormField class="w-full" :label="$t('businessCardInfo.subtitle')" name="total">
				<UInput
					class="w-full"
					size="md"
			...
			<UFormField class="w-full" :label="$t('businessCardInfo.description')" name="total">
				<UTextarea
					class="w-full"
					size="md"
		...
		<UButton :label="$t('buttons.save')" :disabled="!isValid" :loading="isLoading" @click="handleSubmit" />
	</CommonCardWrapper>
</template>
