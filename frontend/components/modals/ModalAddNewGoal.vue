...
import type { FinanceSpecs } from "@/types/finance.ts";
import {
	CalendarDate,
	DateFormatter,
	getLocalTimeZone,
...
const schema = z.object({
	title: z.string({ required_error: t("inputsErrors.required") }),
	total: z.number({ required_error: t("inputsErrors.required") }),
	amount: z.number({ required_error: t("inputsErrors.required") }),
	exchangeId: z
...
const isValid = computed(
	() => schema.safeParse(state).success && untilAt.value,
);
...
<template>
	...
	>
		...
		>
			<UFormField
				class="w-full"
				:label="$t('inputs.goalTitle')"
				name="title"
			>
				<UInput
					class="w-full"
					size="md"
					:placeholder="$t('inputs.goalTitle')"
					v-model="state.title"
				/>
			</UFormField>
			...
			>
				<UPopover>
					...
					>
						{{
							untilAt
								? df.format(untilAt.toDate(getLocalTimeZone()))
								: $t("inputs.untilAt")
						}}
					</UButton>
