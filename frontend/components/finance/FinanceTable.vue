...
import type { IPaginatedResponse, IPagination } from "@/types";
...
interface IProps {
	...
	data: IPaginatedResponse<FinanceTransaction> | null;
}
...
const columns: TableColumn<FinanceTransaction>[] = [
	...
	},
	{
		...
		cell: ({ row }) => {
			return formatDate(
				normalizeDate(row.getValue("createdAt")),
				"D-MM-YYYY",
			);
		},
	},
...
const page = computed(
	() => Math.ceil(props.pagination.offset / props.pagination.limit) + 1,
);
